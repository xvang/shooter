...
public class Menu extends BaseScreen {
    ...
    */
     private void init(){
         ...
         // The options are: Play, Settings, Instructions, ProfileScreen.
         TextButton play = new TextButton("Play", s.assets.buttonStyle);
         TextButton options = new TextButton("Options", s.assets.buttonStyle);
        ...
        profile.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent e, float x, float y) {
                s.setScreen(s.profileScreen);
            }
        });

         //Adding the buttons to the masterTable(declared in BaseScreen)
         masterTable.add(play).pad(20).row();
        // masterTable.add(options).pad(20).row();
         masterTable.add(instructions).pad(20).row();
         masterTable.add(profile).pad(20).row();
